...
class QuestionInputRawInput(BaseSchema):
    """
    Schema for the raw data extracted from the source (e.g., PDF text blocks).
    This serves as the context for the LLM to generate questions.
    """
    content: str = Field(..., min_length=10, description="The raw text extracted from the source.")
    page_number: Optional[int] = Field(None, ge=1, description="Optional reference to the source page.")
    source_ref: Optional[str] = Field(None, description="Filename or URL identifier.")
    #TODO: define JSON structure


class QuestionInputExtractedQuestions(BaseSchema):
    """
    A 'proto-question' generated by AI and refined by the user.
    Matches the structure of QuestionCreate to allow easy migration.
    """
    question_text: str = Field(..., min_length=5, max_length=1000)
    options: list[QuestionOption] = Field(..., min_length=2)
    answer: int = Field(..., ge=0)
    explanations: Optional[list[QuestionExplanation]] = Field(None)
    difficulty: int = Field(5, ge=1, le=10)
    grade: int = Field(None, ge=1, le=13, description="Target school grade (1-13).")
    #TODO: define JSON structure
...
class QuestionInputExtractedQuestionsUpdate(BaseSchema):
    ...
    extracted_questions: list[QuestionInputExtractedQuestions] = Field(
        ...,
        description="The full list of AI-generated question drafts."
    )
...
class QuestionInputCreate(BaseSchema):
    ...
    Used when a user uploads content for question generation.
    """
    user_id: int = Field(..., gt=0, description="The user initiating the generation.")
    subject_id: int = Field(..., gt=0, description="The subject context for the input.")
    topic_id: int = Field(..., gt=0, description="The specific topic the content relates to.")
    grade: int = Field(..., ge=1, le=13, description="Target school grade (1-13).")
    input_type: InputType = Field(..., description="The format of the source (PDF, TEXT, etc.).")

    raw_input: list[QuestionInputRawInput] = Field(
        ...,
        description="List of text segments extracted from the original source."
    )
    extracted_questions: Optional[list[QuestionInputExtractedQuestions]] = Field(
        None,
        description="Drafts generated by the LLM. Usually populated in a second step."
    )
...
class QuestionInputUpdate(BaseSchema):
    ...
    Used when a user uploads content for question generation.
    """
    subject_id: int = Field(None, gt=0, description="The subject context for the input.")
    topic_id: int = Field(None, gt=0, description="The specific topic the content relates to.")
    grade: int = Field(None, ge=1, le=13, description="Target school grade (1-13).")
    input_type: InputType = Field(..., description="The format of the source (PDF, TEXT, etc.).")
...
#TODO: Docstrings, Description and model_validator
